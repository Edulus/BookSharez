...
- **API keys in the browser:** `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, and `GOOGLE_BOOKS_API_KEY` are declared as constants in [js/supabase-config.js](js/supabase-config.js). The Google Books key is restricted to this domain + Books API only in Google Cloud Console. The DeepSeek and ISBNdb keys live only in Supabase Edge Function secrets — never client-side.
