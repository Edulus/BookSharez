...
- **"Pages"** (homepage / dashboard / bookDetail / profilePage) are divs toggled with `style.display`, addressed by **hash routing** (July 4): `#/`, `#/listing/<id>`, `#/book/<bookId>`, `#/profile/<userId>`, `#/dashboard[/<tab>]`. The router block at the top of [js/main.js](js/main.js) is the single hash↔page mapping: page functions *record* their route via `_setRoute()` (with `replace=true` for within-page changes like dashboard tabs); the `hashchange` listener *applies* routes on back/forward and deep links (`_applyRoute()`); `_routeWrites` suppresses echo events. The initial route is applied after the first auth state (so `#/dashboard` deep links work with a restored session). External-search books (no catalog id) are deliberately unrouted. When adding a page, add both a `_setRoute()` call in its show-function and a case in `_applyRoute()`. Verified by [verify-routing.js](verify-routing.js) (serve the dir on port 7654 first).
