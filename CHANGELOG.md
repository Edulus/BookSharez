...
- User UX pass on the homepage's six "How It Works" step cards, which were static (no click affordance despite the hover lift). Four wired to existing flows: **Sign Up Free** → `goRegisterOrDashboard()` (signup form if logged out, straight to the dashboard shelf if already registered — a logged-in user shouldn't see a signup form); **Curate Your Shelf** → `showAddToShelfModal('have')`; **List for Sale** → `showSellModal()`; **Browse & Search** → new `focusHomeSearch()` (scrolls to and focuses the existing homepage search bar — the destination was already on-page). New `.step-card.clickable` CSS class adds `cursor: pointer` only to cards that actually do something.
- **Card 6 ("Books Find New Homes") stays non-clickable** — user call: it's a payoff/value statement describing the outcome of a sale, not a step with its own action, and forcing it onto Browse or Sell would just duplicate another card.
...
- **Verified:** [verify-routing.js](verify-routing.js) gained a `#/members` deep-link + populated-grid check (13→15 checks); full sweep green (routing, mobile, security, bookflow, batchscan, notifications, enrichment) plus a manual screenshot pass against live Supabase data.
