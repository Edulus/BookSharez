...
- [ ] **14. Apply [db/books_isbn_nullable.sql](db/books_isbn_nullable.sql)** — one `ALTER TABLE`: lets catalog books have a NULL isbn, so pre-ISBN era books (surfaced by the scanner's cover path **and multi-book shelf-photo capture**) can be shelved and listed. Until applied, adding a no-ISBN book shows the generic "Couldn't save book" alert (in shelf capture it folds into the "N couldn't be saved" summary); everything else is unaffected. **Now gates two features** — bump priority. Manual test at the bottom of the SQL file.
